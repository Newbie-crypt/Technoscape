...
void SidePlayer::movePlayer() {
    ...
    QList<QGraphicsItem*> verticalHits =
        legs->collidingItems(); // Check for collisions with vertical wall.
    for (QGraphicsItem* item : verticalHits) {
        if (typeid(*item) == typeid(Wall)) {
            if (velocityY >
                0) { // If wall is below, reverse movement and set player state to grounded
                moveBy(0, -velocityY);
                isGrounded = true;
            } else if (velocityY < 0) {
...
void SidePlayer::updateAnimation() {
    ...
    if (currentState != lastState)
        currentFrame = 0; // If it's different from the last state, restart the animation timer.

    switch (currentState) // Rule of thumb for changing the values here. DON'T. Just call me.
    {
    case WALK:
        currentFrame += 4;
        currentFrame = currentFrame % 320; // 16 different Pixmaps, so %320 so the highest we get to
                                           // is 15 with int division (by 20), so on so forth.
    ...
    case DAMAGED:
        ...
        break; // Unnecessary, but to keep code consistent and not check outside.
    }
