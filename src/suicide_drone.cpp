...
suicideDrone::suicideDrone(Player* t) : Enemy(1, ":/assets/Standing_Robot.png", 7) {
    ...
    explosionFrameWidth = spritesheets[AnimationState::Attacking].width() /
                          spritesheet_columns[AnimationState::Attacking];
    explosionFrameHeight = spritesheets[AnimationState::Attacking].height() /
    ...
    for (int j = 0; j < frame_count[AnimationState::Running]; j++) {
        animations[AnimationState::Running].push_back(spritesheets[AnimationState::Running].copy(
            c * frame_width, r * frame_height, frame_width, frame_height));
        c = (c + 1) % spritesheet_columns[AnimationState::Running];
        if (c == 0) r++;
    }

    for (int j = 0; j < frame_count[AnimationState::Attacking]; j++)
        animations[AnimationState::Attacking].push_back(
            spritesheets[AnimationState::Attacking].copy(
                j * explosionFrameWidth, 0, explosionFrameWidth, explosionFrameHeight));
...
void suicideDrone::Chase() {
    ...
    QPointF playerCenter = target->pos() + QPointF(target->boundingRect().width() / 2,
    ...
    // Since in this case speed = magnitude of the velocity, then we can use the unit vector
    // "direction" to find the velocity.
    velocity = speed * direction;

    // If it's moving to the left, flip the sprite horizontally
    // else, don't change it, but if the sprite was already flipped, flip it back to its original
    // form.
    if (direction.x() < 0) {
