...
extern bool paused;
// This constructor is responsible for designing the "Game Over" screen that pops up after the
// player dies
gameOver::gameOver(QGraphicsView* inputView, gameLevel* inputLevel) {
    ...
    // Cyan and magenta text layers are slightly offset behind the white text to create a visual
    // effect that looks attractive :)
    ...
    // Small Glitch Lines that makes the screen attractive to lure the player into try again instead
    // of leaving the game
    ...
    QString gameOverBtnStyle = "QPushButton {"
    ...
    // Timer that slightly shifts the colored text layers and glitch line to keep the Game Over
    // screen visually animated
    ...
    // When the player dies, pause the game, shake the screen, fade to black, then reveal the Game
    // Over overlay
