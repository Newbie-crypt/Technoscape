...
void brute::setHealthBar(BossHealthBar* h) {
    ...
    QObject::connect(health, &BossHealthBar::bossDead, [this]() { isDead = true; });
}
...
brute::brute(Player* t) : Enemy(200, ":/assets/Standing_Robot.png", 5) {
    ...
    // spritesheets[AnimationState::Idle].load(":/assets/OrangeRobot_Idle.png"); (idle is no longer
    // neede)
    ...
    frame_width = spritesheets[AnimationState::Running].width() /
                  spritesheet_columns[AnimationState::Running];
    frame_height =
        spritesheets[AnimationState::Running].height() / spritesheet_rows[AnimationState::Running];

    // All widths and heights of the spritesheets are the same. So, we only need one width, and one
    // height.
    ...
    frame_width *= factor;
    frame_height *= factor;
    spritesheets[AnimationState::Attacking] = spritesheets[AnimationState::Attacking].scaled(
        spritesheet_width, spritesheet_height, Qt::KeepAspectRatio);
    spritesheets[AnimationState::Running] = spritesheets[AnimationState::Running].scaled(
        spritesheet_width, spritesheet_height, Qt::KeepAspectRatio);
    spritesheets[AnimationState::Attacking2] = spritesheets[AnimationState::Attacking2].scaled(
        spritesheet_width, spritesheet_height, Qt::KeepAspectRatio);
    spritesheets[AnimationState::bossSpecialMove] =
        spritesheets[AnimationState::bossSpecialMove].scaled(spritesheet_width, spritesheet_height,
                                                             Qt::KeepAspectRatio);
    spritesheets[AnimationState::bossDeath] = spritesheets[AnimationState::bossDeath].scaled(
        spritesheet_width, spritesheet_height, Qt::KeepAspectRatio);

    legs = new LegHitbox(this);
    QRectF br = boundingRect();
    legs->configure(br.width() / 2, (br.height() / 2) + 50, br.x() + br.width() / 4,
                    br.y() + br.height() / 4);
    ...
    connect(timer, &QTimer::timeout, [this]() {
        ...
        if ((currentAnimationState == AnimationState::Attacking ||
             currentAnimationState == AnimationState::Attacking2) &&
            currentFrame == 4) {
    ...
    QObject::connect(timer2, &QTimer::timeout, [this, timer, timer2]() {
        ...
        if (target->collidesWithItem(this)) {
            ...
            QTimer::singleShot(1000, []() { std::cout << "\n\n\nOne second\n\n\n"; });
