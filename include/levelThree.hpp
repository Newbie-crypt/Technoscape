...
class levelThree : public gameLevel {
    ...
    QPointer<suicideDrone>
        drones[10]; // What made us use QPointer instead of the standard C++ pointer is that
...
class WaveBanner : public QWidget {
    ...
    explicit WaveBanner(QWidget* parent, int w) : QWidget(parent), waveNumber(w) {
        ...
        QTimer::singleShot(0, [this]() {
            if (parentWidget()) move((parentWidget()->width() - SZ) / 2, 18);
        });
    ...
    void paintEvent(QPaintEvent*) override {
        ...
        p.drawArc(cx - r + 4, cy - r + 4, (r - 4) * 2, (r - 4) * 2, arcStart * 16, 110 * 16);
        // Counter-rotating shorter arc for depth.
        p.setPen(QPen(QColor(255, 60, 60, ci(140)), 2, Qt::SolidLine, Qt::RoundCap));
        p.drawArc(cx - r + 4, cy - r + 4, (r - 4) * 2, (r - 4) * 2, (-arcStart + 180) * 16,
                  50 * 16);
        ...
        for (int i = 0; i < 12; i++) {
            ...
            p.drawLine(QPointF(cx + (r - 1) * std::cos(angle), cy + (r - 1) * std::sin(angle)),
                       QPointF(cx + (r - 1 - len) * std::cos(angle),
                               cy + (r - 1 - len) * std::sin(angle)));
        }
        ...
        QString shown2 =
            part2.left(qMin((int)part2.length(), std::max(0, revealed - (int)part1.length())));
        ...
        for (int o : {-2, 0, 2})
            p.drawText(QRect(o, cy - 52, SZ, 26), Qt::AlignCenter, shown1);
        // Crisp pass.
        ...
        for (int o : {-2, 0, 2})
            p.drawText(QRect(o, cy - 16, SZ, 46), Qt::AlignCenter, shown2);
        // Crisp.
