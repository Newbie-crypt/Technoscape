...
class HealthBar : public QWidget {
    ...
    void paintEvent(QPaintEvent*) override {
        ...
        painter.drawPixmap(QRect(0, 0, fillWidth, fill.height()), fill,
...
// Mirrors Weapon's projectile/audio logic, but is positioned relative to scene instead of player
// and fires on its own timer.
class Turret : public QObject, public QGraphicsPixmapItem {
    Q_OBJECT
    private:
    int damage;
    int fireRate;      // Cooldown between timer calls
    int fireDirection; // matches Projectile/Weapon: 1,2,4,5,6,8,9,10 (but we would only need
                       // 1,2,4,and 8 here.
    int randomShot;
    bool canShoot;
    QPixmap turretSheet;
